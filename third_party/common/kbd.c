...
void emu_key_pressed_mod(long key, int mod) {
  if (raw_keycode_func) {
    // Just consume this.
    return;
  }
  ...
  if (ui_enabled) {
    ...
    emux_key_interrupt_mod(key, 1 /* down */, mod);
  }
}

void emu_key_pressed(long key) {
  emu_key_pressed_mod(key, 0);
}

void emu_key_released_mod(long key, int mod) {
  if (raw_keycode_func) {
  ...
  if (ui_enabled) {
    ...
    emux_key_interrupt_mod(key, 0 /* up */, mod);
  }
