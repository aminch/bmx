...
#if defined(RASPPI) && RASPPI != 5
// Circle's legacy VC4/linux addon provides its own pthread ABI, which
// collides with circle-newlib's POSIX typedefs on newer stdlib releases.
...
extern void emu_key_pressed_mod(long keycode, int mod);
extern void emu_key_released_mod(long keycode, int mod);
