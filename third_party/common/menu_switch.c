...
struct s_cfg_flags {
  ...
  int have_audio_out;
  int have_enable_dpi;
  int have_framebuffer_width;
  int have_framebuffer_height;
  int have_framebuffer_depth;
  int have_hdmi_group_cmdline;
  int have_hdmi_mode_cmdline;
  int have_pi5kms_timings;
...
static const char *default_kernel_for_pi_model(int pi_model) {
  int count = sizeof(pi_kernel_specs) / sizeof(pi_kernel_specs[0]);
...
static const char *kernel_suffix_for_machine_class(int machine_class) {
...
static void append_cmdline_option(char *line, const char *key,
                                  const char *value, int *need_space) {
  ...
  strcat(line, new_option);
  *need_space = 1;
}

static void append_pi5kms_timings(char *line, const char *value,
                                  int *need_space) {
  char timings[VALUE_LEN];
  unsigned i;

  snprintf(timings, sizeof timings, "%s", value);
  for (i = 0; timings[i] != '\0'; i++) {
    if (isspace((unsigned char) timings[i])) {
      timings[i] = ',';
    }
  }
  append_cmdline_option(line, "pi5kms_timings", timings, need_space);
}
...
static const char *machine_option_key_for_config_key(const char *key) {
  if (strcmp(key, "hdmi_group:0") == 0 || strcmp(key, "hdmi_group:1") == 0) {
    return "hdmi_group";
  }
  if (strcmp(key, "hdmi_mode:0") == 0 || strcmp(key, "hdmi_mode:1") == 0) {
    return "hdmi_mode";
  }
  return key;
}

static int config_line_is_section(const char *line) {
...
static int config_line_has_key(const char *line, const char *match) {
  char key[KEY_LEN];
  int i = 0;
  ...
  while (*line && *line != '=' && !isspace((unsigned char)*line) && i < KEY_LEN - 1) {
    key[i++] = *line++;
  }
  key[i] = '\0';
  ...
  return *line == '=' && strcmp(key, match) == 0;
}
...
static void apply_override_s(char *line,
  ...
  if (strlen(line) > 0 && line[0] != '#') {
    ...
    struct machine_option* found =
       find_option(machine_option_key_for_config_key(key), head->options);
    char new_option[OPTION_SCRATCH_LEN];
    if (found) {
...
static int apply_override_m(char *line, struct machine_entry *head,
  ...
  if (strlen(line) > 0 && line[0] != '#') {
    ...
    while (pos < total) {
      ...
      if (strcmp(key,"framebuffer_width") == 0) { cfg_flags->have_framebuffer_width = 1; }
      if (strcmp(key,"framebuffer_height") == 0) { cfg_flags->have_framebuffer_height = 1; }
      if (strcmp(key,"framebuffer_depth") == 0) { cfg_flags->have_framebuffer_depth = 1; }
      if (strcmp(key,"hdmi_group") == 0) { cfg_flags->have_hdmi_group_cmdline = 1; }
      if (strcmp(key,"hdmi_mode") == 0) { cfg_flags->have_hdmi_mode_cmdline = 1; }
      if (strcmp(key,"pi5kms_timings") == 0) { cfg_flags->have_pi5kms_timings = 1; }
      ...
      if (strcmp(key, "pi5kms_timings") == 0) {
         ...
         if (found) {
            append_pi5kms_timings(replacement, found->value, &need_space);
         }
      } else if (found) {
    ...
    if (!cfg_flags->have_framebuffer_width) {
       struct machine_option* found = find_option("framebuffer_width", head->options);
       if (found) {
          if (need_space) { strcat(replacement," "); }
          snprintf(new_option, OPTION_SCRATCH_LEN, "%s=%s", found->key, found->value);
          strcat(replacement, new_option);
    ...
    if (!cfg_flags->have_framebuffer_height) {
       struct machine_option* found = find_option("framebuffer_height", head->options);
       if (found) {
          if (need_space) { strcat(replacement," "); }
          snprintf(new_option, OPTION_SCRATCH_LEN, "%s=%s", found->key, found->value);
          strcat(replacement, new_option);
    ...
    if (!cfg_flags->have_framebuffer_depth) {
       struct machine_option* found = find_option("framebuffer_depth", head->options);
       if (found) {
          if (need_space) { strcat(replacement," "); }
          snprintf(new_option, OPTION_SCRATCH_LEN, "%s=%s", found->key, found->value);
          strcat(replacement, new_option);
    ...
    if (!cfg_flags->have_hdmi_group_cmdline) {
       struct machine_option* found = find_option("hdmi_group", head->options);
       if (found) {
          append_cmdline_option(replacement, found->key, found->value, &need_space);
       }
    }
    if (!cfg_flags->have_hdmi_mode_cmdline) {
       struct machine_option* found = find_option("hdmi_mode", head->options);
       if (found) {
          append_cmdline_option(replacement, found->key, found->value, &need_space);
       }
    }
    if (!cfg_flags->have_pi5kms_timings) {
...
static int apply_config(struct machine_entry* head, int pi_model, struct s_cfg_flags *cfg_flags) {
  ...
  const char *default_kernel = default_kernel_for_pi_model(pi_model);
  const char *machine_suffix = kernel_suffix_for_machine_class(head->class);
  ...
  strcpy(kernel_name, default_kernel);
  strcat(kernel_name, machine_suffix);
  ...
  while (fgets(line, CONFIG_TXT_LINE_LEN - 1, fp)) {
    ...
    if (config_line_has_key(line, "kernel")) {
      if (cfg_flags->need_kernel && !cfg_flags->have_kernel) {
    ...
    if (config_line_is_section(line) && cfg_flags->need_kernel && !cfg_flags->have_kernel) {
      fprintf(fp2,"kernel=%s\n", kernel_name);
      cfg_flags->have_kernel = 1;
    }
