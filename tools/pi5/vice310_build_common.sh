...
# Shared Pi5/Pi500 VICE 3.10 build helpers.
...
NEWLIBDIR="$CIRCLE_STDLIB_HOME/install/aarch64-none-circle"
...
TOOLCHAIN_URL="https://developer.arm.com/-/media/Files/downloads/gnu/15.2.rel1/binrel/$TOOLCHAIN_TARBALL"
...
build_circle_stdlib() {
  ...
  rename_wpa_supplicant_sha1_symbols
  make -C libs/circle/lib/sound clean all
...
set_vice310_make_args() {
  ...
  vice_make_args=(
    ...
    "CPP=aarch64-none-elf-gcc -E"
    "CXXCPP=aarch64-none-elf-g++ -E"
...
build_vice310_kernel() {
  ...
  image_dir="$(bmc64_vice310_image_dir pi5)"
  mkdir -p "$image_dir"
  cp "build/pi5/$VICE310_CLASS/kernel_2712.img" \
    "$image_dir/kernel_2712.img.$VICE310_IMAGE_SUFFIX"

  if [ "$VICE310_COPY_DEFAULT" -eq 1 ]; then
    cp "build/pi5/$VICE310_CLASS/kernel_2712.img" \
      "$image_dir/kernel_2712.img"
  fi
}
