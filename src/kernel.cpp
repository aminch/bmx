...
CKernel::CKernel(void)
  ...
  for (int i = 0; i < MAX_USB_DEVICES; i++) m_pKeyboard[i] = 0;
  m_pMouse = 0;
...
void CKernel::SetupUSBKeyboard() {
  ...
  for (unsigned i = 0; i < MAX_USB_DEVICES; i++) {
    ...
    m_pKeyboard[i] = (CUSBKeyboardDevice *)mDeviceNameService.GetDevice(DeviceName, FALSE);
    if (m_pKeyboard[i] != 0) {
      m_pKeyboard[i]->RegisterRemovedHandler(KeyRemovedHandler);
      m_pKeyboard[i]->RegisterKeyStatusHandlerRaw(KeyStatusHandlerRaw);
      num_keyboards++;
    }
  }
